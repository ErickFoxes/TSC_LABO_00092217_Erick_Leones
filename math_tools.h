...
void invertirMatrix(Matrix MOriginal, Matrix &I){
  float determinante_matriz = determinant(MOriginal);
  if(determinante_matriz==0){
    cout<<"Determinante es 0, bye bye :p";
    exit(EXIT_SUCCESS);
  }else{
    Matrix MCofactor;
    cofactors(MOriginal,MCofactor);

    Matrix MTranspuesta;
    transpose(MCofactor, MTranspuesta);
    
    Matrix MInversa;
    productRealMatrix(1/determinante_matriz, MTranspuesta, I);
  }
}